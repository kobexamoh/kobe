<?php 
  include("header.php");
  include("topheading.php");
?>
<h3>about</h3>
<div class="row">
    <div class="five columns">
        <p>Okay, so the nonprofessional stuff:</p>
        <p>I'm a huge soccer fan – <i>Hala Madrid y nada mas</i> and all that. I'm musical: I've played the piano since I was 4 or 5, and the guitar since I was 12. I like my music varied: from metalcore to hip-hop, from UK grime to drum and bass, from indie-alternative to classical (hi, <i>Moonlight Sonata</i>), I'm versatile in my tastes.</p>
        
        <p>I like being well rounded. I was always the kid who could never pick between the arts and the sciences, the kid who wasn't really sure <i>what</i> he wanted to do with his life. What does a jack-of-all-trades end up doing, anyway?</p>
    
        <p>After a lot of interesting life experiences <del>that may or may not have involved a previous life as a biology major</del> I stumbled into the world of frontend development and since then, I've been hooked. It was a long time in the making, if I'm being honest; I never thought that throwing up a blog on Blogger in 7th grade would bring me full circle nearly two decades later to the world of frontend development.</p>
    
        <p>This is probably where I'd use some buzzwords – empathy, crafting-delightful-and-thoughtful-experiences, pixel-perfect-layouts, all of that – but I won't. It wouldn't be me. I don't need buzzwords to tell you that I get a kick out of the frontend experience, and I <i>genuinely</i> mean that. All of the good parts, all of the messy parts (styling for Internet Explorer? In 2020?) to the things that everyone can't seem to come to a consensus on (<i>hello</i>, site architecture and card sorts) I really do get a kick out of building up things that people interact with.</p>

        <p>Fine. Maybe one cliche. That feeling you get when you see your code in the browser – code you can <i>tangibly</i> interact with? I still live for that feeling, it humbles me every day, and I'm stoked on being in a career where I get to have these experiences regularly.</p>
    </div>
    <div class="seven columns">
        <img src="img/headshot-v1.jpg" alt="Self Portrait. Headshot.">
    </div>
</div>

    <h3>Currently</h3>
    <div class="row">
      <div class="four columns info-block">
        <p>Student @ Northern Alberta Institute of Technology (NAIT)</p>
        <p>Program: Digital Media and Information Technology (Web Design & Development concentration)</p>
      </div>
      <div class="eight columns info-block">
          <p>While a full time student, I'm happily interning with the Alberta Language Technology Laboratory (ALT Lab) under the supervision of Dr. Antti Arppe and Eddie Antonio Santos on the <i>itwêwina</i> project – an "intelligent" Plains Cree – English dictionary. As one of the undegraduate software development interns, I'm primarily a frontend developer: I work with Eddie and Dr. Arppe (along with a <i>very</i> talented team of linguists, developers, and PhDs) to make the frontend of the dictionary more robust and usable for those interested in using it.</p>

          <p>To learn more about the project, you can visit the Lab's site <a href="http://altlab.artsrn.ualberta.ca/tools-applications/" target="_blank">here</a>.</p>

          <p class="aside">Please note that as of March 2020, I'm not (actively) looking for new grad/intern roles. If you have something interesting for May 2020 onwards, though, get in touch with me: I might just be interested in hearing more and joining your team.</p>
      </div>
    </div>

    <h4>Previously</h4>
    <div class="row info-block">  
        <div class="four columns">
          <h5 class="role">QA Web Intern</h5>
          <p class="location">NAIT</p>
          <p class="timeline">Jan 2019 – July 2019</p>
        </div>
        <div class="eight columns">
            <p>As an intern with the NAIT web team, my role was a bit different: with a few other interns, we were tasked with helping the web team in their goal of launching NAIT's new website – in six months.</p>
                
            <p>Bouncing between a sandbox environment and a production environment, the interns were responsible for logging bugs, alerting the developers/designers to issues that would impact the user experience, and got to participate in daily standups, too. Our work wasn't just in isolation with the web team: rather, we were in continuous communication with the marketing & communications units and used their feedback to drive some of our QA testing and development processes.</p>


            <p>I even got an Agile Marketing certification – but that's for another time.</p>

            <p class="aside">(Out of respect for the nature of the work done with the team, I can't share all the tools/processes used during my internship. If you're an interested party (e.g. a recruiter, hiring manager, etc), send me a note and I'll make that info accessible for you in some way or form.</p>
        </div>
    </div>
    
    <div class="row info-block">
        <div class="four columns">
          <h5 class="role">Web Content Assistant</h5>
          <p class="location">University of Alberta (Facilities and Operations)</p>
          <p class="timeline">May 2015 – August 2017</p>
        </div>
        <div class="eight columns">
            <p>Oh, Facilities and Operations.</p>
                
            <p>
                In this role, my work focused on web content (duh) strategy and execution. The big task? Helping Facilities and Operations (F&O) transition their web content from an outdated version of their content management system (Sitecore 2) to a newer modern version (Sitecore 3). The catch? It had to be done with <i>minimal</i> disruption to our stakeholders and site visitors – as fast as we could go.</p>

            <p>Oy vey, indeed.</p>
            
            <p>Over the course of 27 months, I worked with my supervisor (the lovely and ever talented Fiona Gamble) to get acquainted with the rules and regulations at F&O, the ins and outs of Sitecore's CMS, and got really good at refining communications' pieces for different audiences. My role was a web content assistant, but – like so many other comms professionals – it encompassed so many other areas. A week of work could have me working in Lighroom (editing photos for our monthly newsletter), switching to Mailchimp to <i>plan</i> the content for said newsletter, bouncing between Google Calendar and Google Docs in creation of a well-timed communications' plan, and sitting in a meeting room with a mechanical engineer or two to learn about how Canadian engineers number and standardize their documents and specifications.</p>

            <p>All in a day's work, really.</p>

            <p>While some of the work I <i>did</i> do for F&O was quite internal (and can't be shared here), some of it was quite public facing! You're welcome to contact me for it; if you're feeling a little <del>masochistic</del> adventurous, you can visit the <a href="https://www.ualberta.ca/vice-president-facilities-operations/index.html" target="_blank">Facilities and Operations' website</a> and guess where I might have been involved in. Have fun!</p>
        </div>
    </div>
    
    <div class="row info-block">
        <div class="four columns">
            <h5 class="role">Production & Analytics Assistant</h5>
            <p class="location">StyleBox Media</p>
            <p class="timeline">June 2014 – August 2015</p>
        </div>
        <div class="eight columns">
            <p>StyleBox Media was my first 'real' job in the tech space, and I couldn't be more grateful to my boss – Paul Steier – for giving me the chance to work with him. As a hybrid production house and media studio, StyleBox was built around creating content for brands, groups, and organizations. Paul was – and probably still is – a big influence on my development as a creative. It's where I perfected how to be a balance of easygoing and goal focused on the job, and it's where I got my first exposure to client management (and, occasionally, conflict resolution during tense moments on set!).</p>
            
            <p>My role with StyleBox was twofold (okay, more than that, but let's be concise!). As a production assistant, I would assist Paul and the other crew members on set – from holding lights to prepping camera subjects to keeping gear from blowing away, I was <i>the</i> PA to end all PAs.</p>

            <p>As an analytics assistant, I helped with all things web related...in terms of Google Analytics. It was my first time ever doing any sort of analytics work, but it sort of came naturally to me. I had never realized that there was a way to digitally quantify all the sorts of questions and queries I'd had about the sites I'd been playing with previously. Being able to answer questions like who visited the sites the most, where the visitors were from, what pages they spent the most time on, why they left and where they left from was <i>extremely</i> cool to me, and again, I'm so grateful that Paul gave a kid barely out of high school an opportunity to learn on the spot like that.</p>
        </div>
    </div>
        
<?php 
  include("footer.php");
?>